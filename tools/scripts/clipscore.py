...
def extract_all_captions(captions, model, device, batch_size=256, num_workers=8):
    ...
    with torch.no_grad():
        for b in tqdm.tqdm(data):
            b_caption = b['caption'].to(device)
            all_text_features[b['idx']] = model.encode_text(b_caption).cpu().numpy()
    all_text_features = np.vstack([x[1] for x in sorted(all_text_features.items(), key=lambda x: x[0])])
...
def extract_all_images(images, model, device, batch_size=64, num_workers=8):
    ...
    with torch.no_grad():
        for b in tqdm.tqdm(data):
            b_image = b['image'].to(device)
            if device == 'cuda':
                b_image = b_image.to(torch.float16)
            all_image_features[b['idx']] = model.encode_image(b_image).cpu().numpy()
    all_image_features = np.vstack([x[1] for x in sorted(all_image_features.items(), key=lambda x: x[0])])
...
def main():
    ...
    candidates = [candidates_map[cid] for cid in image_ids if cid in candidates_map]
    image_paths = [image for image in image_paths if pathlib.Path(image).name in candidates_map]
    ...
    if args.references_json:
        ...
        key = args.score_name
        scores = {image_id: {key: float(clipscore), key+"_caption": caption }
                  for image_id, clipscore, caption in
                  zip(image_ids, per_instance_image_text,candidates)}
        print('CLIPScore: {:.4f}'.format(np.mean([s[key] for s in scores.values()])))
    ...
    if args.save_per_instance:
        with open(args.save_per_instance, 'r') as f:
            prev_scores = json.load(f)
            for prev_score in prev_scores:
                if prev_score in scores:
                    scores[prev_score].update(prev_scores[prev_score]) 
                else:
                    scores[prev_score] = prev_scores[prev_score]
                best = 0
                best_name = ""
                for x in scores[prev_score]:
                    if x.endswith('core') and best < scores[prev_score][x]: 
                        best = scores[prev_score][x]
                        best_name = x
                scores[prev_score]["best_score"] = best
                scores[prev_score]["best_score_name"] = best_name
                scores[prev_score]["best_score_caption"] = scores[prev_score][best_name+'_caption']
